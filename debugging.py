...
class Terrain(object):
    def __init__(self):
        ...
        # perlin noise object
        self.noise = OpenSimplex(seed=10)
    ...
    def mesh(self, offset=0, height=2.5, wf_data=None):
        ...
        if wf_data is not None:
            ...
            wf_data = wf_data[:len(self.xpoints) * len(self.ypoints)]  # Truncate extra data
            wf_data = wf_data.reshape((len(self.xpoints), len(self.ypoints)))   
    ...
    def update(self):
        ...
        try:
            ...
            wf_data = struct.unpack(str(2 * self.CHUNK) + 'B', wf_data)
            wf_data = np.array(wf_data, dtype='b')[::2] + 128
            wf_data = np.array(wf_data, dtype=np.int32) - 128
            wf_data = wf_data * 0.04

            # Ensure wf_data has the correct shape
            wf_data = wf_data[:len(self.xpoints) * len(self.ypoints)]  # Truncate excess
            wf_data = wf_data.reshape((len(self.xpoints), len(self.ypoints)))

        except Exception as e:
            print(f"Error reading audio stream: {e}")
            # Use static data as fallback
            wf_data = np.ones((len(self.xpoints), len(self.ypoints)), dtype=np.float32)
        ...
        # Debugging output
        print(f"Updated mesh with offset: {self.offset}")
